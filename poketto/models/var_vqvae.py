"""
References:
- LDM: https://github.com/CompVis/stable-diffusion/blob/21f890f9da3cfbeaba8e2ac3c425ee9e998d5229/ldm/modules/diffusionmodules/model.py
...
Borrowed from
    - https://github.com/FoundationVision/VAR/blob/main/models/basic_vae.py
    - https://github.com/FoundationVision/VAR/blob/main/models/vqvae.py
"""
...
class VectorQuantizer2(nn.Module):
    ...
    def forward(self, f_BChw: torch.Tensor, ret_usages=False) -> Tuple[torch.Tensor, List[float], torch.Tensor]:
        ...
        with torch.amp.autocast('cuda', enabled=False):
            ...
            for si, pn in enumerate(self.v_patch_nums): # from small to large
                ...
                if self.training:
                    if dist.is_initialized(): handler = dist.all_reduce(hit_V, async_op=True)
    ...
    def f_to_idxBl_or_fhat(self, f_BChw: torch.Tensor, to_fhat: bool, v_patch_nums: Optional[Sequence[Union[int, Tuple[int, int]]]] = None) -> List[Union[torch.Tensor, torch.LongTensor]]:  # z_BChw is the feature from inp_img_no_grad
        with torch.amp.autocast('cuda', enabled=False):
            B, C, H, W = f_BChw.shape
            if f_BChw.dtype != torch.float32: f_BChw = f_BChw.float()
            f_no_grad = f_BChw.detach()
            f_rest = f_no_grad.clone()
            f_hat = torch.zeros_like(f_rest)
...
class VAR_VQVAE(nn.Module):
    ...
    # ===================== `forward` is only used in VAE training =====================
    def inner_forward(self, inp, ret_usages=False):   # -> rec_B3HW, idx_N, loss
    ...
    def loss(self, pred, data, vq_loss):
        gt = data['img']
        loss_recon_mse = (gt - pred).square().mean()
        loss = loss_recon_mse + vq_loss
        return dict(
            loss=loss,
    ...
    def embed_to_img(self, ms_h_BChw: List[torch.Tensor], all_to_max_scale: bool, last_one=False) -> Union[List[torch.Tensor], torch.Tensor]:
        if last_one:
            ...
            return [self.decoder(self.post_quant_conv(f_hat)) for f_hat in self.quantize.embed_to_fhat(ms_h_BChw, all_to_max_scale=all_to_max_scale, last_one=False)]
...
if __name__ == '__main__':
    model = VAR_VQVAE(
        vocab_size=4096, z_channels=32, ch=128, beta=0.25,
